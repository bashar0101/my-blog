...
const PATH = "src/content/ui.json";

export default function StringsEditor() {
  const { draft, setDraft, status, error, save, ready } = useContentDraft<UiDict>(currentUi, {
    paths: [PATH],
    parse: (files) => parseJsonFile(files, PATH, currentUi),
  });
  return <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
    ...
    {Object.keys(draft).map((key) => <CollapsibleSection key={key} testId="string-editor" title={key}><LocalizedField label={key} value={draft[key]!} onChange={(value) => setDraft({ ...draft, [key]: value })} /></CollapsibleSection>)}
    <SaveBar status={status} error={error} ready={ready} onSave={() => save([{ path: PATH, content: JSON.stringify(draft, null, 2) + "\n", encoding: "utf8" }], "content: update interface strings")} />
  </div>;
}
